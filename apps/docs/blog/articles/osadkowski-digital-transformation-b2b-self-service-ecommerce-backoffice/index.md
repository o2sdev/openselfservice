...
While the web portal currently serves as the primary entry point, Osadkowski is considering the introduction of a hybrid mobile application. Instead of developing a separate native frontend, the team is evaluating the option of wrapping the existing React.js application in a WebView-based shell connected to the BFF. This approach reuses the same UI components, routing and domain models across browser, PWA and native mobile touchpoints.
