...
- Medusa.js integration for carts, checkout, customers, and payments — so you can connect to a real commerce backend out of the box.
...
- `StepIndicator` — visual step progress for multi-step checkout.
- `RadioTile` — selectable option tiles (e.g., for shipping or payment methods).
- `AddressFields`, `CartSummary`, `CartItem`, `CartPromoCode`, `QuantityInput` — building blocks for cart and checkout pages.
