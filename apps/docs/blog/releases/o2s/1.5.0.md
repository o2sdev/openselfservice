...
- Global `RolesGuard` and `PermissionsGuard` are now abailable, so all protected endpoints can enforce roles and permissions consistently.
...
- Zendesk integration docs has benn updated for ticket creation, custom fields, and Survey.js-driven forms.
