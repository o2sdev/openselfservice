...
export const getEmailSchema = (errorMessages?: Models.FormField.ErrorMessage[]) => {
    return YupString()
        .required(errorMessages?.find((error) => error.type === 'required')?.description)
        .max(MAX_EMAIL_CHARS, errorMessages?.find((error) => error.type === 'max')?.description)
        .email(errorMessages?.find((error) => error.type === 'email')?.description);
};
...
export const getCompanyTaxIdSchema = (errorMessages?: Models.FormField.ErrorMessage[]) => {
    return YupString()
        .required(errorMessages?.find((error) => error.type === 'required')?.description)
...
export const getPositionSchema = (errorMessages?: Models.FormField.ErrorMessage[]) => {
    return YupString().required(errorMessages?.find((error) => error.type === 'required')?.description);
};
