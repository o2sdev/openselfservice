...
export const TicketListPure: React.FC<TicketListPureProps> = ({ locale, accessToken, ...component }) => {
    const initialFilters: Components.TicketList.Request.GetTicketListComponentQuery = {
        id: component.id,
        offset: 0,
    ...
    const [data, setData] = useState<Components.TicketList.Model.TicketListComponent>(component);
    const [filters, setFilters] = useState(initialFilters);
    const [isPending, startTransition] = useTransition();

    const handleFilter = (data: Partial<Components.TicketList.Request.GetTicketListComponentQuery>) => {
        startTransition(async () => {
            const newFilters = { ...filters, ...data };
            const newData = await sdk.components.getTicketList(newFilters, { 'x-locale': locale }, accessToken);
    ...
    const handleReset = () => {
        startTransition(async () => {
            const newData = await sdk.components.getTicketList(initialFilters, { 'x-locale': locale }, accessToken);
