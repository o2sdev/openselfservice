...
export const NotificationSummaryPure = ({ ...component }) => {
    const { infoCards, layout } = component;
    if (!infoCards || infoCards.length === 0) {
        return null;
    }
    const isVertical = layout === 'vertical';
    const containerClass = cn('w-full gap-6', isVertical ? 'flex flex-col' : 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4');
    return (React.createElement("div", { className: containerClass }, infoCards.map((infoCard, index) => {
        const colorClass = infoCard.variant &&
            Mappings.NotificationSummary.notificationSummaryVariants[infoCard.variant];
