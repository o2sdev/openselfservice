...
- 270355f: refactor(blocks): build the block SDKs on the shared instance

    Every block built an SDK of its own out of the same 15 lines: the API url resolution, the logger settings with an `@ts-expect-error` per environment variable, and a method-by-method pick out of its own factory. They now take the instance from `@o2s/utils.frontend/sdk` and extend it with everything their factory returns, which leaves one `ofetch` client and one logger for all of them instead of 42, and drops the undeclared dependency on `next-runtime-env` that every block relied on.

    `order-details` gains `sdk.blocks.getOrderPdf`: its factory has always defined the method, and only the method-by-method pick left it out.
...
- b7f8177: chore(deps): update dependencies
...
- cb50455: chore(deps): update dependencies
- ee42afd: fix(blocks.product-list): show the request error toast when a product list refetch fails, instead of leaving the rejection unhandled inside the transition
...
- c6ae8e6: removed permission check from `ProductList` block as it should be accessible also for unauthenticated users
